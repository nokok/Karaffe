...
public class MethodDef extends AbstractNode {
	...
	public String vSource() {
		return String.format("%s %s %s %s %s", findModifierNode().vSource(), findReturnTypeNode().vSource(),
				findNameNode().vSource(), findParameterNode().vSource(), findMethodBodyNode().vSource());
	}

	@Override
	public NodeList normalize(NormalizeContext context) {
		Node modifiers = this.findModifierNode();
		Node name = this.findNameNode();
		Node parameters = this.findParameterNode();
		Node returnType = this.findReturnTypeNode();
		Node block = this.findMethodBodyNode().normalize(context);
