...
import org.karaffe.compiler.base.tree.expr.BinaryExpr;
...
public class NewASTVisitor extends KaraffeBaseVisitor<Tree> implements PositionContext {
    ...
    public Tree visitAdditiveExpr(KaraffeParser.AdditiveExprContext ctx) {
        if (ctx.op == null) {
            ctx.multiplicativeExpr().accept(this);
        }
        BinaryExpr expr = new BinaryExpr();
        expr.setLeftExpr(ctx.left.accept(this));
        expr.setOp(Operators.byToken(ctx.op));
        expr.setRightExpr(ctx.right.accept(this));
        return expr;
    }
